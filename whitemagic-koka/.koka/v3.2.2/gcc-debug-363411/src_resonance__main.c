...
#include "src_resonance__main.h"
extern kk_unit_t kk_src_resonance__main__main_fun2(kk_function_t _fself, kk_context_t* _ctx) {
  kk_function_static_drop(_fself,kk_context());
  kk_string_t _x_x3 = kk_string_empty(); /*string*/
  kk_src_resonance_server_loop(_x_x3, kk_integer_from_small(0), 0x1p-1, 0x1.e666666666666p-1, _ctx); return kk_Unit;
}
...
void main_init(kk_context_t* _ctx) {
  kk_src_resonance__main__init(_ctx);
}
void main_run(kk_context_t* _ctx) {
  kk_src_resonance__main__main(_ctx);
}
void main_done(kk_context_t* _ctx) {
  kk_src_resonance__main__done(_ctx);
}
