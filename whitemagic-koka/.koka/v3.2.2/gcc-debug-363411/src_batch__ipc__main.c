...
#include "src_batch__ipc__main.h"
extern kk_unit_t kk_src_batch__ipc__main__main_fun2(kk_function_t _fself, kk_context_t* _ctx) {
  kk_function_static_drop(_fself,kk_context());
  kk_unit_t __ = kk_Unit;
  kk_string_t _x_x3;
  kk_define_string_literal(, _s_x4, 53, "{\"status\":\"started\",\"batch_ipc\":true,\"version\":\"1.0\"}", _ctx)
  _x_x3 = kk_string_dup(_s_x4, _ctx); /*string*/
  kk_std_core_console_printsln(_x_x3, _ctx);
...
void main_init(kk_context_t* _ctx) {
  kk_src_batch__ipc__main__init(_ctx);
}
void main_run(kk_context_t* _ctx) {
  kk_src_batch__ipc__main__main(_ctx);
}
void main_done(kk_context_t* _ctx) {
  kk_src_batch__ipc__main__done(_ctx);
}
