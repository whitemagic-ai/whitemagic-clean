...
# `ipc_bridge_init` -> `ipc_init`, `ipc_bridge_publish` -> `ipc_publish`, `ipc_bridge_status` -> `ipc_status`
# `import whitemagic_rs` -> `import whitemagic_rust`
# `rs.ipc_bridge_init(node_name)` -> no return value or PyResult, it either passes or raises

new_code = """\"\"\"Iceoryx2 IPC Bridge - Cross-process zero-copy communication.
...
with open('whitemagic/core/ipc_bridge.py', 'w') as f:
    f.write(new_code)

print("Patched python ipc_bridge.py")
