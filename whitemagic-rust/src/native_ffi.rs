...
fn galactic_batch_score_native<'py>(
    ...
    for item in memories.iter() {
        ...
        let importance: f64 = mem_dict.get_item("importance")?.unwrap().extract::<f64>().unwrap_or(0.5);
        let neuro_score: f64 = mem_dict.get_item("neuro_score")?.unwrap().extract::<f64>().unwrap_or(1.0);
        let emotional_valence: f64 = mem_dict.get_item("emotional_valence")?.unwrap().extract::<f64>().unwrap_or(0.0);
        let recall_count: i64 = mem_dict.get_item("recall_count")?.unwrap().extract::<i64>().unwrap_or(0);
        let is_protected: bool = mem_dict.get_item("is_protected")?.unwrap().extract::<bool>().unwrap_or(false);
...
fn association_mine_native<'py>(
    ...
    for item in texts.iter() {
        ...
        let keywords: Vec<String> = text.to_lowercase()
            .split_whitespace()
            .filter(|w| w.len() > 3)
        ...
        for j in (i + 1)..memory_count {
            ...
            if total_unique > 0 {
                ...
                if overlap_score >= min_sc {
                    ...
                    overlap_dict.set_item("shared_keywords", 
                        kw1.iter().filter(|k| kw2.contains(k)).cloned().collect::<Vec<_>>())?;
    ...
    overlaps.sort_by(|a, b| {
        let a_score: f64 = a.get_item("overlap_score").unwrap().unwrap().extract().unwrap();
        let b_score: f64 = b.get_item("overlap_score").unwrap().unwrap().extract().unwrap();
