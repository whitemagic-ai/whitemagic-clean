...
use std::sync::{Arc, RwLock, atomic::{AtomicU8, AtomicU64, Ordering}};
...
pub use event_bus::{EventBus, OrchestrationEvent, EventType, Priority};
pub use health::{HealthMonitor, HealthStatus, HealthState};
pub use registry::{SubsystemRegistry, SubsystemHandle, SubsystemConfig};
pub use lifecycle::{LifecycleManager, SystemState};
pub use dag::{DependencyGraph, DagExecutor};
pub use primitives::{
    CircuitBreaker, CircuitBreakerConfig, CircuitBreakerRegistry, CircuitBreakerStats, CircuitState,
    CircuitBreakerError, RetryConfig, RetryExecutor, RetryError,
    BackpressureController, BackpressureConfig, BackpressureGuard, BackpressureError, LoadMetrics,
};
...
impl ResonanceConductor {
    ...
    pub fn register_subsystem(&self, config: SubsystemConfig) -> SubsystemId {
        ...
        self.emit_event(EventType::SubsystemRegistered, Some(id), None, HashMap::new());
    ...
    pub fn start(&self) -> Result<(), ConductorError> {
        ...
        self.state.store(SystemState::Awakening as u8, Ordering::SeqCst);
        ...
        self.state.store(SystemState::Resonant as u8, Ordering::SeqCst);
        ...
        self.emit_event(
            ...
            HashMap::from([
                ...
                ("subsystems".to_string(), serde_json::json!(self.registry.count())),
            ]),
        );
    ...
    pub fn shutdown(&self) -> Result<(), ConductorError> {
        ...
        self.state.store(SystemState::Dormant as u8, Ordering::SeqCst);
    ...
    fn execute_bootstrap_sequence(&self) -> Result<(), ConductorError> {
        ...
        let sequence = executor.topological_sort()
    ...
    fn execute_shutdown_sequence(&self) -> Result<(), ConductorError> {
        ...
        let executor = DagExecutor::new(&graph);
        let mut sequence = executor.topological_sort()
            .map_err(|e| ConductorError::DependencyError(e.to_string()))?;
...
impl std::fmt::Display for ConductorError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::BootstrapFailed(id, e) => write!(f, "Bootstrap failed for subsystem {}: {}", id, e),
...
mod tests {
    ...
    fn test_event_emission() {
        ...
        let sequence = conductor.emit_event(
            EventType::Bootstrap,
            None,
            None,
            HashMap::new(),
        ...
        let sequence2 = conductor.emit_event(
