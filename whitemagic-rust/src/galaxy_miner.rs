...
pub fn mine_access_patterns(db_path: &str, min_frequency: i64) -> PyResult<Vec<AccessPattern>> {
    let conn = Connection::open(db_path)
        .map_err(|e| PyErr::new::<pyo3::exceptions::PyIOError, _>(format!("DB open failed: {}", e)))?;
    ...
    }).map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("Query map failed: {}", e)))?;
    
    for row in rows {
        if let Ok((id, access_count)) = row {
            memory_ids.push(id);
...
pub fn mine_cache_candidates(
    ...
    let rows = stmt.query_map(rusqlite::params![min_access, min_importance], |row| {
        Ok((
            row.get::<_, String>(0)?,
            row.get::<_, i64>(1)?,
            row.get::<_, f64>(2)?,
    ...
    for row in rows {
        if let Ok((id, access_count, importance)) = row {
            memory_ids.push(id);
            total_access += access_count;
...
pub fn mine_semantic_clusters(
    ...
    for row in rows {
        if let Ok((memory_id, tags_str, importance)) = row {
    ...
    for (tag, memories) in tag_to_memories.iter() {
        if memories.len() >= min_cluster_size {
            ...
            let avg_importance = memories.iter().map(|(_, imp)| imp).sum::<f64>() / memories.len() as f64;
