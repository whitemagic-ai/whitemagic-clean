...
def main():
    ...
    try:
        # Let's give it max 3 minutes since reading contents takes a little longer
        while not BLACKBOARD["completed"] and time.time() - start_time < 180:
            time.sleep(1)
            sys.stdout.write(f"\\r[Blackboard] Intersting Files Processed: {BLACKBOARD['total_files_scanned']} | Target Directories Found: {len(BLACKBOARD['directories'])}")
            sys.stdout.flush()
    ...
    with open(report_path, "w") as f:
        ...
        f.write(f"**Highly Interesting Target Files Sent to Ring:** {BLACKBOARD['total_files_scanned']}\\n\\n")
