...
def main():
    ...
    # Let Elixir print directly to terminal to see what's happening
    elixir_proc = subprocess.Popen(
        ['mix', 'run', 'swarm_scout_slow.exs'],
    ...
    try:
        ...
        while not BLACKBOARD["completed"] and time.time() - start_time < 300:
            ...
            sys.stdout.flush()
            if elixir_proc.poll() is not None and not BLACKBOARD["completed"]:
                print("\\n[Director] Elixir process died unexpectedly!")
                break
