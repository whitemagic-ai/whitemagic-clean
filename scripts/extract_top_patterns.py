...
print(f"   These have multi-source validation and high importance.")
print(f"   Safe for automatic application without review.")

print(f"\n2. **Manual Review Queue (0.60-0.77)**: {len(high)} patterns")
print(f"   High confidence but below auto-apply threshold.")
print(f"   Quick human review before application.")

print(f"\n3. **Pattern Library (0.50-0.60)**: {len(medium)} patterns")
print(f"   Store for reference and future cross-validation.")
print(f"   May gain confidence as more data arrives.")

# Specific insights from top patterns
print(f"\n4. **Key Insights from Top 10**:")

if any('windsurf' in p['tag'].lower() for p in top_10):
    print(f"   • Windsurf conversation patterns detected")
    print(f"     → Mine session transcripts for optimization insights")

if any('sangha' in p['tag'].lower() or 'intelligence' in p['tag'].lower() for p in top_10):
    print(f"   • Strategic planning patterns detected")
    print(f"     → Extract architectural decisions for pattern library")

if any('mcp' in p['tag'].lower() for p in top_10):
    print(f"   • MCP-related patterns detected")
    print(f"     → Analyze tool usage patterns for optimization")
...
print(f"Ready for integration with AdaptiveSystem")
print(f"{'='*80}")
