...
# Fix the unused Activity import error by making sure the UI was actually injected
# The previous string replace might have missed if the exact string wasn't found
if "Activity" in content and "Activity size={12}" not in content:
    # Let's forcefully inject it right before the version info
    pulse_ui = """
...
"""
    content = re.sub(r'      \{/\* System Status \*/\}', pulse_ui + '\n      {/* System Status */}', content)

with open('nexus/src/components/status/StatusBar.tsx', 'w') as f:
    f.write(content)

print("Fixed StatusBar TS error")
