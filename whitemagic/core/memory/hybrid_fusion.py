    ...
    def fuse_rrf(self, 
        ...
        for rank, result in enumerate(vector_results):
            ...
            if mid not in candidates:
                candidates[mid] = RetrievalCandidate(
                    memory_id=mid,
                    title=result.get('title', ''),
                    content=result.get('content', '')
                )
            candidates[mid].vector_score = 1.0 / (self.rrf_k + rank + 1)
        ...
        for rank, result in enumerate(bm25_results):
            ...
            candidates[mid].bm25_score = 1.0 / (self.rrf_k + rank + 1)
        ...
        for rank, result in enumerate(graph_results):
            ...
            candidates[mid].graph_score = 1.0 / (self.rrf_k + rank + 1)
