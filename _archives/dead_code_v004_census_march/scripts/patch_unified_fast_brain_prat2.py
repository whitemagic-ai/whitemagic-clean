...
fun handle-event(ring-addr: int64, embed-addr: int64, graph-addr: int64, event: string) : io ()
  ...
  if event.contains("query") then
    ...
    val tool = if event.contains("search_memories") then "search_memories" elif event.contains("create_memory") then "create_memory" else "unknown"
...
content = re.sub(r'fun handle-event\(ring-addr: int64, embed-addr: int64, graph-addr: int64, event: string\) : io \(\).*?    println\("{\\\\"status\\\\":\\\\"telemetry_processed.*?"\)\n', patch.strip('\n') + '\n', content, flags=re.DOTALL)

with open(file_path, "w") as f:
    f.write(content)

print("Unified Fast Brain updated with PRAT Routing (syntax fix)")
