...
# Fix the handler effect composition syntax. 
# Koka effect handlers can be nested by passing the action directly.
patch = """
fun server-loop(): io ()
  with handler
  ...
  if line != "" then
    if line.contains("\\"quit\\"") then ()
      ...
      val response = with handle-prat-dispatch-production
