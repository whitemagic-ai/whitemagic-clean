...
fun extract-prat-tool(event: string) : string
  if event.contains("search_memories") then "search_memories" else {
    if event.contains("create_memory") then "create_memory" else "unknown"
  }
...
print("Unified Fast Brain handle-event rewritten cleanly (nested braces)")
