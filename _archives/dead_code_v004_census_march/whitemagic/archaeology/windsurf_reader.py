...
class WindsurfConversationReader:
    ...
    def read_conversation(self, path: str) -> Conversation:
        ...
        for msg in messages:
            if msg.role == "user" and len(msg.content) > 10:
                conv.title = msg.content[:100] + "..." if len(msg.content) > 100 else msg.content
                break
    ...
    def index_conversations(self) -> Dict[str, Any]:
        """
        Build an index of all conversations for fast lookup.
        
        Returns:
            Index dictionary with conversation metadata
        """
        index = {
            "version": "1.0",
        ...
        for conv_meta in self.list_conversations():
            try:
                conv = self.read_conversation(conv_meta["path"])
    ...
    def search_conversations(self, query: str) -> List[Dict[str, Any]]:
        """
        Search across all conversations for a query string.
            ...
            try:
                ...
                for i, msg in enumerate(conv.messages):
                    if query_lower in msg.content.lower():
                        ...
                        idx = msg.content.lower().find(query_lower)
                        start = max(0, idx - 100)
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        total_size = sum(c["size_bytes"] for c in conversations)
        ...
        return {
            ...
            "base_path": str(self.base_path) if self.base_path else None,
            "newest": conversations[0] if conversations else None,
            "oldest": conversations[-1] if conversations else None,
        }


# Convenience functions
def get_windsurf_reader() -> WindsurfConversationReader:
    """Get a WindsurfConversationReader instance."""
    return WindsurfConversationReader()

def list_windsurf_conversations() -> List[Dict[str, Any]]:
    """List all Windsurf conversations."""
    return get_windsurf_reader().list_conversations()

def read_windsurf_conversation(path: str) -> Conversation:
    """Read a specific Windsurf conversation."""
    return get_windsurf_reader().read_conversation(path)
