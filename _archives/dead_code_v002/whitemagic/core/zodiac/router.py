...
class ZodiacRouter:
    ...
    def _classify_operation(self, operation: str) -> OperationType:
        """Classify operation into type for routing using shared logic."""
        from whitemagic.core.intelligence.synthesis.shared import classify_intent

        intent = classify_intent(operation)
        try:
            return OperationType(intent)
        except ValueError:
    ...
    def _build_routing_table(self) -> dict[str, Callable]:
